...
Kn = 1/300;     % Waning rate of natural immunity (1/day)
...
global alpha1 chi omega beta1 beta2 Cm epsilonm kappa epsa1 epsa2 epsa3 ...
    SumEpsLi SumEpsL2i Sumthetai Kv Kv1 Kv2 eps3i p2 gamma2 alphaB chiB omegaB
...
dS1_dt = Lambda + 2*Kv*Wv + 2*Kn*W - lambd(t).*(1-Cmnew(t)*epsilonm).*S1 ...
    - SumEpsai(t) - d*S1;
dE1_dt = lambd(t).*(1-Cmnew(t)*epsilonm).*S1 - w*E1 - d*E1;
dI1_dt = p1*w*E1 - (alpha1+gamma1)*I1 - d*I1;
dA1_dt = (1-p1)*w*E1 - gamma1*A1 - d*A1;
dH1_dt = alpha1*I1 - omega*H1 - d*H1;
dV1_dt = SumEpsai(t) - SumEpsLi.*lambd(t).*(1-Cmnew(t)*epsilonm).*V1 ...
    - Sumthetai.*V1 - 2*Kv1*V1 - d*V1;
dV2_dt = Sumthetai.*V1 - SumEpsL2i.*lambd(t).*(1-Cmnew(t)*epsilonm).*V2 ...
    - 2*Kv2*V2 - d*V2;
dEB_dt = SumEpsLi.*lambd(t).*(1-Cmnew(t)*epsilonm).*V1 + SumEpsL2i.*lambd(t).*(1-Cmnew(t)*epsilonm).*V2 ...
    + lambd(t).*(1-Cmnew(t)*epsilonm)*eps3i.*V3 - w*EB - d*EB;
...
dV3_dt = rho4(t)*Wv - lambd(t).*(1-Cmnew(t)*epsilonm)*eps3i.*V3 - d*V3;
